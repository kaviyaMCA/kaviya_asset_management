...
const getbyrefid = async (req, res) => {
  ...
  try {
    const data = await AssetDetails.findAll({
      where: {
        asset_ref_id: refid, // Equivalent to "asset_ref_id = ?" in raw SQL
      },
    });
    apiResponse.status = "Success";
    apiResponse["code"] = 200;
...
const getEmpAsset = async (req, res) => {
  const empid = req.body.emp_id;
  console.log(empid);
  try {
    const data = await AssetDetails.findAll({
      where: {
        emp_id: empid, // Equivalent to "asset_status = ?" in raw SQL
      },
    });
    res.json(data);
  } catch (err) {
...
const setScrapStatus = async (req, res) => {
  const { id } = req.body;
  const apiResponse = {};
  try {
    const result = await AssetDetails.update(
      { asset_status: 3 }, // Update these columns
      { where: { id } } // Apply this condition
    );
    apiResponse["status"] = "success";
    apiResponse["code"] = 200;
